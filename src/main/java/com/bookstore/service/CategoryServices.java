...
public class CategoryServices {
	...
	public void updateCategory() throws ServletException, IOException {
		...
		if (categoryByName != null && categoryByName.getCategoryId() != categoryById.getCategoryId()) {
			String message = "Could not update category. User with category "+categoryName+" already exists";
			request.setAttribute("message", message);
	...
	public void deleteCategory() throws ServletException, IOException {
		...
		String message;
		if(numberOfBooks > 0) {
			message = "Could not delete the category (ID: %d) as it currently contains some books";
			message = String.format(message, numberOfBooks);
		}else {
			categoryDAO.delete(categoryId);
			message = "Category with ID " +categoryId+ " been deleted successfully";
